...
**轉速擊退加成 `spinKnockback`**：攻擊方自旋（轉速）越高，碰撞後把對手彈得越遠（夾制之後的額外推力，
隨自旋衰減而遞減，不會無限暴衝）；場地後台可調。
...
四圍分工：攻擊=扣血+擊退、防禦=減傷+減擊退、重量=血量上限(`hpBase×weight`)+抗出界、續航=自旋衰減慢。
**血量基準 `hpBase`** 是場地參數（場地後台可調，可隨場地存不同血量經濟）；舊場地未設則用引擎預設 `HP_BASE`。

**勝負**：出界 (ring-out) / 停轉 (spin-out，自旋耗盡) / 擊破 (ko，血量耗盡) / 時間到比體力 (timeout)。
...
- **衝擊 (blast)**：打出夠猛的一擊時，機率把對手**彈開一段距離 + 造成傷害**（可順勢擊出界）。

每招**獨立觸發機率**（`rushChance` / `blastChance`）。回放在發動瞬間畫光環 + 標籤（突進! / 衝擊!）。
必殺技為 opt-in，不影響 `npm run balance` 的基礎平衡。所有數值集中成 **`SpecialConfig`**
（`DEFAULT_SPECIAL`）→ 存 `specialStore.ts`(localStorage) → **「陀螺後台 › 必殺技數值」按招分組可即時調**。
